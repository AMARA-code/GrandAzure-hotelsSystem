...
export default function NewBookingPage() {
  ...
  const categoryOptions = useMemo(() => {
    const cats = Array.from(
      new Set(roomTypes.map((r) => r.type_category).filter(Boolean))
    ).sort()
    return cats
  }, [roomTypes])

  const filteredRoomTypes = useMemo(() => {
    if (categoryFilter === 'all') return roomTypes
    return roomTypes.filter((r) => r.type_category === categoryFilter)
  ...
  const selectedRoomType = useMemo(() => {
    ...
    const id = parseInt(form.room_type_id, 10)
    return roomTypes.find((r) => r.room_type_id === id) ?? null
  }, [form.room_type_id, roomTypes])

  const priceBreakdown = useMemo(() => {
    if (!selectedRoomType || nightsCount <= 0) {
      return { subtotal: 0, tax: 0, total: 0, ratePerNight: 0 }
    }
    const ratePerNight = Number(selectedRoomType.base_price)
    const subtotal = ratePerNight * nightsCount
    const tax = Math.round(subtotal * 0.16)
    const total = subtotal + tax
    return { subtotal, tax, total, ratePerNight }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!form.room_type_id || !selectedRoomType) {
        toast.error('Select a room type for this property')
        return
      }

      if (!form.rate_plan_id) {
        toast.error('No active rate plan for this property — add one in the database or pick another property')
        return
      }
      ...
      const maxOcc = selectedRoomType.max_occupancy != null
        ? Number(selectedRoomType.max_occupancy)
        : null
      const party = parseInt(form.adults, 10) + parseInt(form.children, 10)
      if (maxOcc != null && party > maxOcc) {
        toast.error(`This room type allows at most ${maxOcc} guests`)
        return
      }

      const hotelIdNum = parseInt(form.hotel_id, 10)
      if (selectedRoomType.hotel_id !== hotelIdNum) {
        toast.error('Selected room type does not belong to this property')
        return
      }
      ...
      // Build guest detail payload (only non-empty values)
      ...
      const roomTypeId = selectedRoomType.room_type_id
      ...
      if (availableRoom) {
        ...
        if (brErr) console.error('booking_rooms insert:', brErr)
      } else {
        toast.info(
          'Booking saved. Assign a room from Rooms when one is available — totals use the selected room type rate.'
        )
      }
      ...
      >
        ...
        <div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Room types and rates load for the selected property.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                {ratePlans.length === 0 ? (
                  <option value="">No plans — check database</option>
                ) : (
                  ratePlans.map((rp) => (
                    <option key={rp.rate_plan_id} value={String(rp.rate_plan_id)}>
                      {rp.plan_name}
                    </option>
                  ))
                )}
                ...
                <option value="">
                  {catalogLoading
                    ...
                    : filteredRoomTypes.length
                      ? 'Select room type'
                      : 'No room types for this property'}
                </option>
                {filteredRoomTypes.map((rt) => (
                  <option key={rt.room_type_id} value={String(rt.room_type_id)}>
                    {rt.type_name} · {formatCurrency(Number(rt.base_price))}/night ·{' '}
                    {(rt.type_category ?? '').replace(/_/g, ' ') || '—'}
                  </option>
                ))}
              </select>
              {selectedRoomType?.max_occupancy != null && (
                <p className="text-xs text-slate-500 mt-1">
                  Max occupancy: {selectedRoomType.max_occupancy} guests
                </p>
              )}
            </div>
          </div>
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-2">
            Charges: nightly rate × nights + 16% tax (computed automatically).
          </p>
        </div>
          ...
          <div className="rounded-2xl border border-azure-100 bg-azure-50/80 p-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-azure-700/90">
                  {formatCurrency(priceBreakdown.ratePerNight)} × {nightsCount} night{nightsCount !== 1 ? 's' : ''}
                </span>
