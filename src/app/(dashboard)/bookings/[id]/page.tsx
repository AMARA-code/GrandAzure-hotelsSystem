    ...
    <div className="space-y-6 pb-8">
      ...
      >
        <div className="flex items-center gap-4">
          <Link
            href="/bookings"
            className="w-10 h-10 rounded-xl border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  {booking.guest?.vip_status !== 'none' && (
                    <span className={cn(
                      ...
                      {booking.guest?.vip_status?.toUpperCase()} Member
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    {booking.guest?.nationality ?? 'N/A'}
                  </div>
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    <Star className="w-4 h-4 text-gold-400" />
                    {booking.loyalty_points_earned} points earned
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            <h2 className="font-bold text-slate-900 text-lg mb-5 flex items-center gap-2">
              <BedDouble className="w-5 h-5 text-azure-500" />
              Stay Details
            </h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[
                ...
                {
                  label: 'Nights',
                  value: String(booking.total_nights),
                  icon: Moon,
                ...
                {
                  label: 'Guests',
                  value: `${booking.adults} Adults${booking.children > 0
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {br.room_type?.type_name} • Floor {br.room?.floor_number}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-slate-900">
                      {formatCurrency(br.rate_per_night)}
                    </p>
          ...
          >
            <h2 className="font-bold text-slate-900 text-lg mb-4 flex items-center gap-2">
              <Building2 className="w-5 h-5 text-azure-500" />
              Property
            </h2>
            <div className="flex items-center gap-4">
        ...
        <div className="space-y-6">
          ...
          >
            <h2 className="font-bold text-slate-900 text-lg mb-5 flex items-center gap-2">
              <Receipt className="w-5 h-5 text-azure-500" />
              Payment Summary
            </h2>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-800">
                  {formatCurrency(booking.total_amount - booking.tax_amount)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
            ...
            {booking.rate_plan && (
              <div className="mt-4 p-3 rounded-xl bg-azure-50 border border-azure-100">
                <p className="text-xs text-azure-600 font-semibold mb-0.5">
                  Rate Plan
                </p>
                <p className="text-sm font-bold text-azure-800">
                  {booking.rate_plan.plan_name}
                </p>
                <p className="text-xs text-azure-600 capitalize">
                  {booking.rate_plan.meal_plan?.replace('_', ' ')}
                </p>
              </div>
            )}
          </motion.div>
          ...
          >
            <h2 className="font-bold text-slate-900 text-lg mb-4 flex items-center gap-2">
              <Globe className="w-5 h-5 text-azure-500" />
              Booking Source
            </h2>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-800">
                  {booking.channel?.channel_name}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Type</span>
                <span className="font-semibold text-slate-800 capitalize">
                  {booking.channel?.channel_type?.replace('_', ' ')}
                </span>
              </div>
              <div className="flex justify-between text-sm">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Link
                href={`/bookings/${booking.booking_id}/payment`}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 border border-slate-100 transition-colors text-left group"
              >
              ...
              <Link
                href={`/guests/${booking.guest?.guest_id}`}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 border border-slate-100 transition-colors text-left group"
              >
